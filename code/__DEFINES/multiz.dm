...
#define GET_TURF_BELOW(turf) ( \
	(turf.flags_turf & RESERVATION_TURF) ? SSmapping.get_reservation_from_turf(turf)?.get_turf_below(turf) : \
...
#define GET_TURF_ABOVE(turf) ( \
	(turf.flags_turf & RESERVATION_TURF) ? SSmapping.get_reservation_from_turf(turf)?.get_turf_above(turf) : \
