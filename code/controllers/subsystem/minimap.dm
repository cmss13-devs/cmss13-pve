...
/datum/tacmap/drawing/tgui_interact(mob/user, datum/tgui/ui)
	...
	if(is_xeno && xeno.hive.see_humans_on_tacmap && targeted_ztrait != ZTRAIT_MARINE_MAIN_SHIP)
		allowed_flags |= MINIMAP_FLAG_USCM|MINIMAP_FLAG_PMC|MINIMAP_FLAG_UPP|MINIMAP_FLAG_TWE|MINIMAP_FLAG_CLF|MINIMAP_FLAG_IMPERIAL_GUARD
...
/proc/get_minimap_flag_for_faction(faction)
	switch(faction)
		...
		if(FACTION_IMPERIAL_GUARD)
			return MINIMAP_FLAG_IMPERIAL_GUARD
		if(FACTION_YAUTJA)
