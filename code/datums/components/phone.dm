...
/datum/component/phone/Initialize(...)
	...
	handle_initial_variables(arglist(args))
...
/datum/component/phone/proc/handle_initial_variables(phone_category, phone_color, phone_id, phone_icon, holder)
	src.phone_category = phone_category
	src.phone_color = phone_color
	src.phone_id = phone_id
	src.phone_icon = phone_icon
...
/datum/component/phone/proc/call_phone(mob/user, calling_phone_id)
	...
	calling_phone.calling_phone = src
	SEND_SIGNAL(calling_phone.holder, COMSIG_ATOM_PHONE_RINGING)

	post_call_phone(user, calling_phone_id)

	//START_PROCESSING(SSobj, src) - handle ringing - Morrow
...
/datum/component/phone/proc/reset_call(timeout = FALSE, recursed = FALSE)
	...
	SEND_SIGNAL(calling_phone.holder, COMSIG_ATOM_PHONE_STOPPED_RINGING)

	//STOP_PROCESSING(SSobj, T) - Stop ringing - Morrow
...
/datum/component/phone/proc/recall_handset()
	...
	SEND_SIGNAL(calling_phone.holder, COMSIG_ATOM_PHONE_HUNG_UP)
...
/datum/component/phone/proc/handle_speak(message, datum/language/message_language, mob/speaker, direct_talking = TRUE)
	...
	log_say("TELEPHONE: [key_name(speaker)] [direct_talking ? "" : "was picked up "]on Phone '[phone_id]' to '[calling_phone.phone_id]' said '[message]'")
...
/datum/component/phone/virtual/handle_initial_variables(phone_category, phone_color, phone_id, phone_icon, holder, virtual_user)
	src.phone_category = phone_category
	src.phone_color = phone_color
	src.phone_id = phone_id
	src.phone_icon = phone_icon
