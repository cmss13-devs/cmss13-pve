...
/datum/fire_support/proc/initiate_fire_support(turf/target_turf, mob/user)
	...
	if(portrait_type && initiate_title && initiate_screen_message)
		...
		for(var/mob/mob_receiver in alert_receivers)
			...
			mob_receiver.play_screen_text("<span class='langchat' style=font-size:24pt;text-align:left valign='top'><u>[uppertext(name)]</u></span><br>" + pick(initiate_screen_message), new /atom/movable/screen/text/screen_text/potrait(null, null, initiate_title, 'icons/ui_icons/screen_alert_images.dmi', portrait_type))
