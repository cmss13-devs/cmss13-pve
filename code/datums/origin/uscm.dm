...
/datum/origin/uscm/aw/generate_human_name(gender = MALE)
	switch(gender)
		if(FEMALE)
			return capitalize(pick(GLOB.first_names_female)) + " A.W. " + pick(GLOB.weapon_surnames)
		if(MALE)
			return capitalize(pick(GLOB.first_names_male)) + " A.W. " + pick(GLOB.weapon_surnames)
		if(PLURAL)
			return capitalize(pick(pick(GLOB.first_names_male), pick(GLOB.first_names_female))) + " A.W. " + pick(GLOB.weapon_surnames)
