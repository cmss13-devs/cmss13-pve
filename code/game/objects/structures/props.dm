...
/obj/structure/prop/invuln/lore_computer/attack_hand(mob/user)
	pointer++
	playsound(src, "keyboard_all", 15, 1)
	switch(lore_blurb)
		if(1)
			switch(pointer)
				if(1)
					visible_message(SPAN_NOTICE("Welcome to <b>UNET</b>."))
				if(2)
					visible_message(SPAN_NOTICE("Current network unique identifier: <b>CG-1413</b>."))
				if(3)
					visible_message(SPAN_NOTICE("Current network <b>node</b> unique identifier: <b>CG-CMP-LSE</b>."))
				if(4)
					visible_message(SPAN_NOTICE("Pinging network node <b>CG-CMP-ABL</b>..."))
				if(5)
					visible_message(SPAN_NOTICE("...     "))//preventing dumb message stacking im so smart
				if(6)
					visible_message(SPAN_WARNING("Node <b>CG-CMP-ABL</b> not responding."))
				if(7)
					visible_message(SPAN_NOTICE("Pinging network node <b>CG-CMP-ESY</b>..."))
				if(8)
					visible_message(SPAN_NOTICE("...      "))
				if(9)
					visible_message(SPAN_WARNING("Node <b>CG-CMP-ESY</b> not responding."))
				if(10)
					visible_message(SPAN_NOTICE("...          "))
				if(11)
					visible_message(SPAN_NOTICE("Pinging network node <b>CG-HQ-1</b>..."))
				if(12)
					visible_message(SPAN_NOTICE("..."))
				if(13)
					visible_message(SPAN_NOTICE("... "))
				if(14)
					visible_message(SPAN_NOTICE("...  "))
				if(15)
					visible_message(SPAN_WARNING("Node <b>CG-HQ-1</b> not responding."))
				if(16)
					visible_message(SPAN_NOTICE("Pinging network node <b>WY-OR-STL</b>..."))
				if(17)
					visible_message(SPAN_NOTICE("...               "))
				if(18)
					visible_message(SPAN_NOTICE("Received response packets from node <b>WY-OR-STL</b>."))
				if(19 to 26)
					visible_message(SPAN_NOTICE("Decrypting received data from node <b>WY-OR-STL</b>. Please wait."))
				if(27)
					visible_message(SPAN_NOTICE("<b>Displaying recorded logs</b>."))
					pointer = 0
		if(2)
			switch(pointer)
				if(1)
				...
				if(5)
					visible_message(SPAN_NOTICE("...     "))//preventing dumb message stacking im so smart
				if(6)
					visible_message(SPAN_NOTICE("Receceived response packets from node <b>CG-CMP-ABL</b>."))
				if(7)
					visible_message(SPAN_NOTICE("Pinging network node <b>CG-CMP-ESY</b>..."))
				if(8)
					visible_message(SPAN_NOTICE("...      "))
				if(9)
					visible_message(SPAN_NOTICE("Receceived response packets from node <b>CG-CMP-ESY</b>."))
				if(10)
					visible_message(SPAN_NOTICE("...          "))
				if(11)
					visible_message(SPAN_NOTICE("Pinging network node <b>CG-HQ-1</b>..."))
				...
				if(14)
					visible_message(SPAN_NOTICE("...  "))
				if(15)
					visible_message(SPAN_NOTICE("Receceived response packets from node <b>CG-HQ-1</b>."))
				if(16)
					visible_message(SPAN_NOTICE("Displaying received message:"))
				if(17 to 23)
					visible_message(SPAN_NOTICE("'AWAIT PHONE CONTACT. CPT. MCKAY'"))
				if(24)
					visible_message(SPAN_NOTICE("<b>Displaying recorded logs</b>."))
					pointer = 0