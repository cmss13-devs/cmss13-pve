	...
	switch(action)
		...
		*/
		...
		if("toggle_airlock")
			...
			var/signal_id = params["signal_id"]
			for(airlock_door in GLOB.machines)
		...
		if("toggle_door_control")//Nothing fancy here, changes the button's id so it cannot be used, or changes it back.
			...
			for(door_controller in GLOB.machines)
			...
			*/
			...
			switch(dest_actions[1]) //The action is in position 1.
				...
				if("break_all_machines")
					...
					for(any_machine as anything in GLOB.machines)
				...
				if("break_gear_vendors")
					...
					for(gear_vendor in GLOB.machines)
				...
				if("break_normal_vendors")
					...
					for(regular_vendor in GLOB.machines)
				...
				if("break_computers")
					...
					for(computer_terminal in GLOB.machines)
				...
				if("break_apcs")
					...
					for(power_controller in GLOB.machines)
				...
				if("break_lights")
					...
					for(light_bulb in GLOB.machines)
				...
				if("break_cameras")
					...
					for(overhead_camera in GLOB.machines)
				...
				if("break_comm_towers")
					...
					for(comm_tower in GLOB.machines)
				...
				if("break_window_doors")
					...
					for(window_door in GLOB.machines)
