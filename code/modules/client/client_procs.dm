...
/client/Destroy()
	...
	if(CLIENT_IS_STAFF(src))
		...
		REDIS_PUBLISH("byond.access", "event_type" = "logout", "key" = src.key, "remaining" = length(adm["total"]), "afk" = length(adm["afk"]))
...
/client/proc/notify_login()
	...
	if(CLIENT_IS_STAFF(src))
		...
		var/list/adm = get_admin_counts(R_MOD)
		REDIS_PUBLISH("byond.access", "event_type" = "login", "key" = src.key, "remaining" = length(adm["total"]), "afk" = length(adm["afk"]))
