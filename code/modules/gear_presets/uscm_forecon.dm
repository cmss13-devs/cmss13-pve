...
/datum/equipment_preset/uscm/forecon/proc/add_forecon_weapon(mob/living/carbon/human/new_human)
	...
	switch(random_gun)
		...
		if(3)
			...
			new_human.equip_to_slot_or_del(new /obj/item/ammo_magazine/rifle/m49a/heap(new_human), WEAR_IN_ACCESSORY) //Basic model is not great with basic rounds, some HEAP makes it not quite as bad
