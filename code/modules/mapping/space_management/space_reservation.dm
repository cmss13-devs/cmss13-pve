...
/datum/turf_reservation/proc/calculate_cordon_turfs(turf/bottom_left, turf/top_right)
	...
	for(var/turf/cordon_turf as anything in possible_turfs)
		if(!(cordon_turf.flags_turf & UNUSED_RESERVATION_TURF))
			return FALSE
...
/datum/turf_reservation/proc/generate_cordon()
	for(var/turf/cordon_turf as anything in cordon_turfs)
		...
		cordon_turf.flags_turf &= ~UNUSED_RESERVATION_TURF
...
/datum/turf_reservation/proc/make_repel(turf/pre_cordon_turf)
	...
	RegisterSignal(pre_cordon_turf, list(COMSIG_PARENT_QDELETING, COMSIG_TURF_RESERVATION_RELEASED), PROC_REF(on_stop_repel))
...
/datum/turf_reservation/proc/_reserve_area(width, height, zlevel)
	...
	for(var/i in avail)
		CHECK_TICK
		BL = i
		if(!(BL.flags_turf & UNUSED_RESERVATION_TURF))
			continue
		if(BL.x + width > world.maxx || BL.y + height > world.maxy)
			continue
		TR = locate(BL.x + width - 1, BL.y + height - 1, BL.z)
		if(!(TR.flags_turf & UNUSED_RESERVATION_TURF))
			continue
		final = block(BL, TR)
		if(!final)
			continue
		...
		for(var/I in final)
			var/turf/checking = I
			if(!(checking.flags_turf & UNUSED_RESERVATION_TURF))
				passing = FALSE
				break
	...
	for(var/i in final)
		...
		T.flags_turf = (T.flags_turf | RESERVATION_TURF) & ~UNUSED_RESERVATION_TURF
...
/datum/turf_reservation/transit/make_repel(turf/pre_cordon_turf)
	...
	RegisterSignal(pre_cordon_turf, COMSIG_TURF_ENTERED, PROC_REF(space_dump))

/datum/turf_reservation/transit/stop_repel(turf/pre_cordon_turf)
	..()

	UnregisterSignal(pre_cordon_turf, COMSIG_TURF_ENTERED)
