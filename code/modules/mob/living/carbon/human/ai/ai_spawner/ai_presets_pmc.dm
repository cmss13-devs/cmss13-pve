/datum/human_ai_equipment_preset/pmc
	faction = FACTION_PMC

/datum/human_ai_equipment_preset/pmc/standard
	name = "PMC Rifleman"
	desc = "The standard unit of any PMC operation. Plenty of variety in what they're armed with."
	path = /datum/equipment_preset/pmc/standard

/datum/human_ai_equipment_preset/pmc/medic
	name = "PMC - Medic"
	desc = "Medical unit of any PMC operation. Similar weapons to the rifleman."
	path = /datum/equipment_preset/pmc/medic

/datum/human_ai_equipment_preset/pmc/leader
	name = "PMC - Leader"
	desc = "The leader element of any PMC operation. Similar weapons to the rifleman."
	path = /datum/equipment_preset/pmc/leader

/datum/human_ai_equipment_preset/pmc/gunner
	name = "PMC - Gunner"
	desc = "The heavy weapons element to any PMC operation. Armed with a smartgun, deadly."
	path = /datum/equipment_preset/pmc/gunner

/datum/human_ai_equipment_preset/pmc/sniper
	name = "PMC - Sniper"
	desc = "The supporting fire element to any PMC operation. Armed with an Anti-mat rifle, very deadly."
	path = /datum/equipment_preset/pmc/sniper

/datum/human_ai_equipment_preset/pmc/technician
	name = "PMC - Techician"
	desc = "The fortifying element to any PMC operation. Has mini-turret and G-2 electroshock grenades."
	path = /datum/equipment_preset/pmc/technician

/datum/human_ai_equipment_preset/pmc/crewman
	name = "PMC - Crewman"
	desc = "Armed with an SMG this is the PMC analog of a vehicle crewman."
	path = /datum/equipment_preset/pmc/crewman

/datum/human_ai_equipment_preset/pmc/doctor
	name = "PMC - Trauma Surgeon"
	desc = "A civillian doctor working alongside W-Y PMCs, armed with VP70 for self-defense. (Don't shoot, I'm civi-!)"
	path = /datum/equipment_preset/pmc/doctor

/datum/human_ai_equipment_preset/pmc/detainer
	name = "PMC - Security Enforcer"
	desc = "An experienced security member of W-Y PMC. Armed with corporate white M41A mark II."
	path = /datum/equipment_preset/pmc/pmc_detainer

/datum/human_ai_equipment_preset/pmc/riot_control
	name = "PMC - Crowd Control Specialist"
	desc = "Heavily equipped riot control unit. Armed with ES-7 Supernova Electrostatic Shockgun."
	path = /datum/equipment_preset/pmc/pmc_riot_control

/datum/human_ai_equipment_preset/pmc/medical_investigator
	name = "PMC - Medical Investigator"
	desc = "PMC guard medical expert. Usually called for after-fight hostage treatment."
	path = /datum/equipment_preset/pmc/pmc_med_investigator

/datum/human_ai_equipment_preset/pmc/lead_investigator
	name = "PMC - Lead Investigator"
	desc = "PMC guard commander. Detective and military police warden in one flask."
	path = /datum/equipment_preset/pmc/pmc_lead_investigator

/datum/human_ai_equipment_preset/pmc/commando_standard
	name = "PMC Commando - Standard"
	desc = "W-Y PMC finest. These one are not so heavily armed as usual, but don't get cocky while fightin them."
	path = /datum/equipment_preset/pmc/commando/standard/low_threat

/datum/human_ai_equipment_preset/pmc/commando_gunner
	name = "PMC Commando - Smartgunner"
	desc = "A commando equipped with L56D smartgun"
	path = /datum/equipment_preset/pmc/commando/gunner/low_threat

/datum/human_ai_equipment_preset/pmc/commando_dogcatcher
	name = "PMC Commando - Dog Catcher"
	desc = "A commando equipped with apesuit for maximum protection against xenos."
	path = /datum/equipment_preset/pmc/commando/dogcatcher

/datum/human_ai_equipment_preset/pmc/commando_leader
	name = "PMC Commando - Leader"
	desc = "The best, leading the best. Comes with fancy black and red armor, and AP magazines for M41A2 MK2."
	path = /datum/equipment_preset/pmc/commando/leader/low_threat

/datum/human_ai_equipment_preset/pmc/synth
	name = "PMC - Synthetic"
	desc = "A PMC support synthetic, unarmed."
	path = /datum/equipment_preset/pmc/synth

/datum/human_ai_equipment_preset/pmc/synth_combat
	name = "PMC - Security Android"
	desc = "A highly illegal PMC security android. GMs should not be using this willynilly."
	path = /datum/equipment_preset/synth/working_joe/security_android/pmc

/datum/human_ai_equipment_preset/pmc/director
	name = "PMC - Director"
	desc = "PMC dicrector armed with a pistol. The one and only PMC who usually stays far away from active frontline."
	path = /datum/equipment_preset/pmc/director
