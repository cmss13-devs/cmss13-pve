...
/datum/action/xeno_action/activable/headbite/use_ability(atom/target_atom)
	...
	if(!(HAS_TRAIT(target_carbon, TRAIT_KNOCKEDOUT) || target_carbon.stat == UNCONSCIOUS || (locate(/datum/effects/crit) in marine_human.effects_list))) //called knocked out because for some reason .stat seems to have a delay .
