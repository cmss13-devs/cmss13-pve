...
/obj/item/ammo_magazine/flamer_fire_act(damage, datum/cause_data/flame_cause_data)
	if(current_rounds < 1)
		...
		if(severity > 0)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(explosion), loc, -1, ((severity > 4) ? 0 : -1), clamp(severity, 0, 1), clamp(severity, 0, 2), 1, 0, 0, flame_cause_data, TRUE), max(5 - severity, 2))
