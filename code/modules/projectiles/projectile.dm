...
/obj/proc/get_projectile_hit_boolean(obj/projectile/specific_projectile)
	if(!density)
		return FALSE
...
/obj/proc/calculate_cover_hit_boolean(obj/projectile/specific_projectile, distance = 0, cade_direction_correct = FALSE)
	if(istype(specific_projectile.shot_from, /obj/item/hardpoint)) //anything shot from a tank gets a bonus to bypassing cover
		distance -= 3
	...
	var/effective_accuracy = specific_projectile.get_effective_accuracy()
	...
	#if DEBUG_HIT_CHANCE
	to_world(SPAN_DEBUG("([name] as cover) Distance travelled: [specific_projectile.distance_travelled]  |  Effective accuracy: [effective_accuracy]  |  Hit chance: [hitchance]"))
	#endif

	return prob(hitchance)

/obj/structure/machinery/get_projectile_hit_boolean(obj/projectile/specific_projectile)

	if(src == specific_projectile.original && layer > ATMOS_DEVICE_LAYER) //clicking on the object itself hits the object
		var/hitchance = specific_projectile.get_effective_accuracy()

		#if DEBUG_HIT_CHANCE
		to_world(SPAN_DEBUG("([name]) Distance travelled: [specific_projectile.distance_travelled]  |  Effective accuracy: [hitchance]  |  Hit chance: [hitchance]"))
		#endif
	...
	if(!throwpass)
		return TRUE
	var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
	if(ammo_flags & AMMO_IGNORE_COVER)
		return FALSE

	var/distance = specific_projectile.distance_travelled

	if(flags_atom & ON_BORDER) //windoors
		if(specific_projectile.dir & reverse_direction(dir))
			distance-- //no bias towards "inner" side
			if(ammo_flags & AMMO_STOPPED_BY_COVER)
				return TRUE
		else if( !(specific_projectile.dir & dir) )
			return FALSE //no effect if bullet direction is perpendicular to barricade
	else
		distance--

	return calculate_cover_hit_boolean(specific_projectile, distance)


/obj/structure/get_projectile_hit_boolean(obj/projectile/specific_projectile)
	if(src == specific_projectile.original && layer > ATMOS_DEVICE_LAYER) //clicking on the object itself hits the object
		var/hitchance = specific_projectile.get_effective_accuracy()
	...
	var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
	if(ammo_flags & AMMO_IGNORE_COVER && src != specific_projectile.original)
		return FALSE
	...
	if(flags_atom & ON_BORDER) //barricades, flipped tables
		...
		distance--
		if(climbable)
			for(var/obj/structure/S in get_turf(specific_projectile))
				if(S && S.climbable && !(S.flags_atom & ON_BORDER)) //if a projectile is coming from a window frame or table, it's guaranteed to pass the next window frame/table
					return FALSE
	return calculate_cover_hit_boolean(specific_projectile, distance, cade_direction_correct)


/obj/item/get_projectile_hit_boolean(obj/projectile/specific_projectile)

	if(specific_projectile && src == specific_projectile.original) //clicking on the object itself. Code copied from mob get_projectile_hit_chance
...
/obj/vehicle/get_projectile_hit_boolean(obj/projectile/specific_projectile)

	if(src == specific_projectile.original) //clicking on the object itself hits the object
		var/hitchance = specific_projectile.get_effective_accuracy()

		#if DEBUG_HIT_CHANCE
		to_world(SPAN_DEBUG("([specific_projectile.name]) Distance travelled: [specific_projectile.distance_travelled]  |  Effective accuracy: [hitchance]  |  Hit chance: [hitchance]"))
		#endif
...
/obj/structure/window/get_projectile_hit_boolean(obj/projectile/specific_projectile)
	var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
	if(ammo_flags & AMMO_ENERGY)
		return FALSE
	else if(!(flags_atom & ON_BORDER) || (specific_projectile.dir & dir) || (specific_projectile.dir & reverse_direction(dir)))
		return TRUE

/obj/structure/machinery/door/poddoor/railing/get_projectile_hit_boolean(obj/projectile/specific_projectile)
	return src == specific_projectile.original

/obj/effect/alien/egg/get_projectile_hit_boolean(obj/projectile/specific_projectile)
	return src == specific_projectile.original

/obj/effect/alien/resin/trap/get_projectile_hit_boolean(obj/projectile/specific_projectile)
	return src == specific_projectile.original

/obj/item/clothing/mask/facehugger/get_projectile_hit_boolean(obj/projectile/specific_projectile)
	return src == specific_projectile.original
...
/mob/living/proc/get_projectile_hit_chance(obj/projectile/specific_projectile)
	...
	var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
	if((body_position == LYING_DOWN && !(ammo_flags & AMMO_PRONETARGET)|| HAS_TRAIT(src, TRAIT_NO_STRAY)) && src != specific_projectile.original)
		return FALSE
	if(ammo_flags & AMMO_XENO)
		if((status_flags & XENO_HOST) && HAS_TRAIT(src, TRAIT_NESTED))
			return FALSE

	. = specific_projectile.get_effective_accuracy()

	if(body_position == LYING_DOWN && stat)
		. += 15 //Bonus hit against unconscious people.

	if(isliving(specific_projectile.firer))
		var/mob/living/shooter_living = specific_projectile.firer
		if(!can_see(shooter_living,src))
			. -= 15 //Can't see the target (Opaque thing between shooter and target)

/mob/living/carbon/human/get_projectile_hit_chance(obj/projectile/specific_projectile)
	. = ..()
	if(.)
		var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
		if(SEND_SIGNAL(specific_projectile, COMSIG_BULLET_CHECK_MOB_SKIPPING, src) & COMPONENT_SKIP_MOB\
			|| specific_projectile.runtime_iff_group && get_target_lock(specific_projectile.runtime_iff_group)\
		)
			return FALSE
		if(mobility_aura)
			. -= mobility_aura * 5
		var/mob/living/carbon/human/shooter_human = specific_projectile.firer
...
/mob/living/carbon/xenomorph/get_projectile_hit_chance(obj/projectile/specific_projectile)
	. = ..()
	if(.)
		var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
		if(SEND_SIGNAL(specific_projectile, COMSIG_BULLET_CHECK_MOB_SKIPPING, src) & COMPONENT_SKIP_MOB\
		...
		if(ammo_flags & AMMO_SKIPS_ALIENS)
			var/mob/living/carbon/xenomorph/X = specific_projectile.firer
			if(!istype(X))
				return FALSE
...
/mob/living/silicon/robot/drone/get_projectile_hit_chance(obj/projectile/specific_projectile)
...
/atom/proc/bullet_act(obj/projectile/specific_projectile)
	return FALSE

/mob/dead/bullet_act(/obj/projectile/specific_projectile)
	return FALSE

/mob/living/bullet_act(obj/projectile/specific_projectile)
	if(!specific_projectile)
		return

	var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
	var/damage = specific_projectile.calculate_damage()
	if(specific_projectile.ammo.debilitate && stat != DEAD && ( damage || (ammo_flags & AMMO_IGNORE_RESIST) ) )
		apply_effects(arglist(specific_projectile.ammo.debilitate))

	. = TRUE
	bullet_message(specific_projectile, damaging = damage)
	if(damage)
		apply_damage(damage, specific_projectile.ammo.damage_type, specific_projectile.def_zone, 0, 0, specific_projectile)
		specific_projectile.play_hit_effect(src)

	SEND_SIGNAL(specific_projectile, COMSIG_BULLET_ACT_LIVING, src, damage, damage)


/mob/living/carbon/human/bullet_act(obj/projectile/specific_projectile)
	if(!specific_projectile)
		return

	if(isxeno(specific_projectile.firer))
		var/mob/living/carbon/xenomorph/X = specific_projectile.firer
		if(X.can_not_harm(src))
			bullet_ping(specific_projectile)
			return -1

	var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
	if(specific_projectile.weapon_cause_data && specific_projectile.weapon_cause_data.cause_name)
		var/mob/M = specific_projectile.weapon_cause_data.resolve_mob()
		if(istype(M))
			M.track_shot_hit(specific_projectile.weapon_cause_data.cause_name, src)

	var/damage = specific_projectile.calculate_damage()
	var/damage_result = damage

	if(SEND_SIGNAL(src, COMSIG_HUMAN_PRE_BULLET_ACT, specific_projectile) & COMPONENT_CANCEL_BULLET_ACT)
		return

	flash_weak_pain()
	if(specific_projectile.ammo.stamina_damage)
		apply_stamina_damage(specific_projectile.ammo.stamina_damage, specific_projectile.def_zone, ARMOR_ENERGY) // Stamina damage is energy

	//Shields
	if( !(ammo_flags & AMMO_ROCKET) ) //No, you can't block rockets.
		if(prob(75) && check_shields(damage * 0.65, "[specific_projectile]") ) // Lower chance to block bullets
			specific_projectile.ammo.on_shield_block(src)
			bullet_ping(specific_projectile)
			return

	var/obj/limb/organ = get_limb(check_zone(specific_projectile.def_zone)) //Let's finally get what organ we actually hit.
	if(!organ)
	...
	if( damage > 0 && !(ammo_flags & AMMO_IGNORE_ARMOR) )
		...
		switch(specific_projectile.ammo.damage_type)
			if(BRUTE)
		...
		damage_result = armor_damage_reduction(GLOB.marine_ranged, damage, armor, specific_projectile.ammo.penetration)

		if(damage_result <= 5)
			to_chat(src,SPAN_XENONOTICE("Your armor absorbs the force of [specific_projectile]!"))
		if(damage_result <= 3)
			damage_result = 0
			bullet_ping(specific_projectile)
			visible_message(SPAN_AVOIDHARM("[src]'s armor deflects [specific_projectile]!"))
			if(specific_projectile.ammo.sound_armor) playsound(src, specific_projectile.ammo.sound_armor, 50, 1)

	if(specific_projectile.ammo.debilitate && stat != DEAD && ( damage || ( ammo_flags & AMMO_IGNORE_RESIST) ) )  //They can't be dead and damage must be inflicted (or it's a xeno toxin).
	...
	bullet_message(specific_projectile) //We still want this, regardless of whether or not the bullet did damage. For griefers and such.

	if(SEND_SIGNAL(src, COMSIG_HUMAN_BULLET_ACT, damage_result, ammo_flags, specific_projectile) & COMPONENT_CANCEL_BULLET_ACT)
		return

	specific_projectile.play_hit_effect(src)
	if(damage || (ammo_flags & AMMO_SPECIAL_EMBED))

		var/splatter_dir = get_dir(specific_projectile.starting, loc)
		handle_blood_splatter(splatter_dir)

		. = TRUE
		apply_damage(damage_result, specific_projectile.ammo.damage_type, specific_projectile.def_zone, firer = specific_projectile.firer)

		if(specific_projectile.ammo.shrapnel_chance > 0 && prob(specific_projectile.ammo.shrapnel_chance + floor(damage / 10)))
			if(ammo_flags & AMMO_SPECIAL_EMBED)
				specific_projectile.ammo.on_embed(src, organ)

			var/obj/item/shard/shrapnel/new_embed = new specific_projectile.ammo.shrapnel_type
			var/obj/item/large_shrapnel/large_embed = new specific_projectile.ammo.shrapnel_type
	...
	SEND_SIGNAL(specific_projectile, COMSIG_POST_BULLET_ACT_HUMAN, src, damage, damage_result)

//Deal with xeno bullets.
/mob/living/carbon/xenomorph/bullet_act(obj/projectile/specific_projectile)
	if(!specific_projectile || !istype(specific_projectile))
		return
	...
	if((ammo_flags & AMMO_FLAME) && (caste.fire_immunity & (FIRE_IMMUNITY_NO_IGNITE|FIRE_IMMUNITY_NO_DAMAGE)))
		...
		bullet_message(specific_projectile, damaging = FALSE)
		return

	if(isxeno(specific_projectile.firer) && ammo_flags & (AMMO_ACIDIC|AMMO_XENO)) //Xenomorph shooting spit. Xenos with thumbs and guns can fully FF.
		var/mob/living/carbon/xenomorph/X = specific_projectile.firer
		if(X.can_not_harm(src))
			bullet_ping(specific_projectile)
			return -1
	...
	if(damage > 0 && !(ammo_flags & AMMO_IGNORE_ARMOR))
		...
		var/list/damagedata = list(
			"damage" = damage,
			"armor" = armor,
			"penetration" = specific_projectile.ammo.penetration,
			"armour_break_pr_pen" = specific_projectile.ammo.pen_armor_punch,
			"armour_break_flat" = specific_projectile.ammo.damage_armor_punch,
			"armor_integrity" = armor_integrity,
			"direction" = specific_projectile.dir,
		)
	...
	bullet_message(specific_projectile) //Message us about the bullet, since damage was inflicted.

	if(mob_flags & AI_CONTROLLED)
		handle_ai_shot(specific_projectile)

	if(SEND_SIGNAL(src, COMSIG_XENO_BULLET_ACT, damage_result, ammo_flags, specific_projectile) & COMPONENT_CANCEL_BULLET_ACT)
		return

	if(damage)
		//only apply the blood splatter if we do damage
		handle_blood_splatter(get_dir(specific_projectile.starting, loc))

		apply_damage(damage_result,specific_projectile.ammo.damage_type, specific_projectile.def_zone) //Deal the damage.
		if(length(xeno_shields))
			specific_projectile.play_shielded_hit_effect(src)
		else
			specific_projectile.play_hit_effect(src)
	...
	SEND_SIGNAL(specific_projectile, COMSIG_BULLET_ACT_XENO, src, damage, damage_result)

	return TRUE

/turf/bullet_act(obj/projectile/specific_projectile)
	if(SEND_SIGNAL(src, COMSIG_TURF_BULLET_ACT, specific_projectile) & COMPONENT_BULLET_ACT_OVERRIDE)
		return

	if(!specific_projectile || !density)
	...
	for(var/mob/living/L in src)
		if(L in specific_projectile.permutated)
			continue
		if(prob(L.get_projectile_hit_chance(specific_projectile)))
			mobs_list += L

	if(length(mobs_list))
		var/mob/living/picked_mob = pick(mobs_list) //Hit a mob, if there is one.
		if(istype(picked_mob))
			picked_mob.bullet_act(specific_projectile)
			return
	return

// walls can get shot and damaged, but bullets (vs energy guns) do much less.
/turf/closed/wall/bullet_act(obj/projectile/specific_projectile)
	. = ..()
	var/damage = specific_projectile.damage
	if(damage < 1)
		return
	var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
	...
	if(ammo_flags & AMMO_BALLISTIC)
		current_bulletholes++
	take_damage(damage, specific_projectile.firer)

/turf/closed/wall/almayer/research/containment/bullet_act(obj/projectile/specific_projectile)
	if(specific_projectile)
		var/ammo_flags = specific_projectile.ammo.flags_ammo_behavior | specific_projectile.projectile_override_flags
...
/obj/bullet_act(obj/projectile/specific_projectile)
	bullet_ping(specific_projectile)
	return TRUE

/obj/item/bullet_act(obj/projectile/specific_projectile)
	bullet_ping(specific_projectile)
	if(specific_projectile.ammo.damage_type == BRUTE)
		explosion_throw(specific_projectile.damage/2, specific_projectile.dir, 4)
	return TRUE

/obj/structure/surface/table/bullet_act(obj/projectile/specific_projectile)
	bullet_ping(specific_projectile)
	health -= floor(specific_projectile.damage/2)
	if(health < 0)
...
/atom/proc/bullet_ping(obj/projectile/specific_projectile, pixel_x_offset = 0, pixel_y_offset = 0)
	if(!specific_projectile || !specific_projectile.ammo.ping)
		return

	if(specific_projectile.ammo.sound_bounce) playsound(src, specific_projectile.ammo.sound_bounce, 30)
	var/image/I = image('icons/obj/items/weapons/projectiles.dmi', src, specific_projectile.ammo.ping, 10)
	var/offset_x = clamp(specific_projectile.pixel_x + pixel_x_offset, -10, 10)
	var/offset_y = clamp(specific_projectile.pixel_y + pixel_y_offset, -10, 10)
	I.pixel_x += round(rand(-4,4) + offset_x, 1)
	I.pixel_y += round(rand(-4,4) + offset_y, 1)

	var/matrix/rotate = matrix()
	rotate.Turn(specific_projectile.angle)
	I.transform = rotate
...
/mob/proc/bullet_message(obj/projectile/specific_projectile, damaging = TRUE)
	if(!specific_projectile)
		return
	if(damaging && COOLDOWN_FINISHED(src, shot_cooldown))
		visible_message(SPAN_DANGER("[src] is hit by the [specific_projectile.name] in the [parse_zone(specific_projectile.def_zone)]!"), \
			SPAN_HIGHDANGER("[isxeno(src) ? "We" : "You"] are hit by the [specific_projectile.name] in the [parse_zone(specific_projectile.def_zone)]!"), null, 4, CHAT_TYPE_TAKING_HIT)
		COOLDOWN_START(src, shot_cooldown, 1 SECONDS)

	var/shot_from = specific_projectile.shot_from ? " from \a [specific_projectile.shot_from]" : ""
	last_damage_data = specific_projectile.weapon_cause_data
	if(specific_projectile.firer && ismob(specific_projectile.firer))
		var/mob/firingMob = specific_projectile.firer
		...
		if(ishuman(firingMob) && ishuman(src) && faction == firingMob.faction && !A?.statistic_exempt) //One human shot another, be worried about it but do everything basically the same //special_role should be null or an empty string if done correctly
			if(!istype(specific_projectile.ammo, /datum/ammo/energy/taser))
				GLOB.round_statistics.total_friendly_fire_instances++
				var/ff_msg = "[key_name(firingMob)] shot [key_name(src)] with \a [specific_projectile][shot_from] in [get_area(firingMob)] [ADMIN_JMP(firingMob)] [ADMIN_PM(firingMob)]"
				var/ff_living = TRUE
				if(src.stat == DEAD)
					ff_living = FALSE
				msg_admin_ff(ff_msg, ff_living)
				if(ishuman(firingMob) && specific_projectile.weapon_cause_data)
					var/mob/living/carbon/human/H = firingMob
					H.track_friendly_fire(specific_projectile.weapon_cause_data.cause_name)
			else
			...
			msg_admin_attack("[key_name(firingMob)] shot [key_name(src)] with \a [specific_projectile][shot_from] in [get_area(firingMob)] ([firingMob.x],[firingMob.y],[firingMob.z]).", firingMob.x, firingMob.y, firingMob.z)
		attack_log += "\[[time_stamp()]\] <b>[key_name(firingMob)]</b> shot <b>[key_name(src)]</b> with \a <b>[specific_projectile]</b>[shot_from] in [get_area(firingMob)]."
		firingMob.attack_log += "\[[time_stamp()]\] <b>[key_name(firingMob)]</b> shot <b>[key_name(src)]</b> with \a <b>[specific_projectile]</b>[shot_from] in [get_area(firingMob)]."
		return

	attack_log += "\[[time_stamp()]\] <b>[specific_projectile.firer ? specific_projectile.firer : "SOMETHING??"]</b> shot <b>[key_name(src)]</b> with a <b>[specific_projectile]</b>[shot_from]"
	msg_admin_attack("[specific_projectile.firer ? specific_projectile.firer : "SOMETHING??"] shot [key_name(src)] with \a [specific_projectile][shot_from] in [get_area(src)] ([loc.x],[loc.y],[loc.z]).", loc.x, loc.y, loc.z)
